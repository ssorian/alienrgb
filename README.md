...
## Optional post-resume restoration

After every fully completed live `set-all`, `alienrgb` atomically records that command's color as the only restorable profile. It writes exactly six lowercase hexadecimal digits plus one newline to `$XDG_STATE_HOME/alienrgb/last-set-all-color`, or to `$HOME/.local/state/alienrgb/last-set-all-color` when `XDG_STATE_HOME` is unset. The state directory is mode `0700` and the atomically replaced file is mode `0600`. Dry runs, argument or validation errors, transport preflight failures, and partial transport failures never update it. An unresolved state root or persistence failure is reported explicitly; if persistence fails after all 49 operations, the live transport has still occurred but the command returns an error.

The optional systemd integration in [`contrib/systemd`](contrib/systemd/README.md) runs as the named non-root instance user in the static `alienrgb` group. Its boot instance runs before `display-manager.service` in the `graphical.target` transaction, restoring the last profile before login; its resume instance becomes active for `sleep.target` and restores in `ExecStop` after suspend or hibernate. The helper validates the state strictly, performs a bounded sequence of read-only `doctor --json` readiness checks, and then executes the complete confirmed `set-all` command exactly once with every existing confirmation flag. It does not retry transport. Because there is no reliable RGB readback, restoration always reapplies the entire recorded profile. Before the first fully successful `set-all`, no state exists and post-resume handling exits successfully without writing hardware.
...
Live execution accepts only typed color intent. The sealed transport regenerates `encode_equal_color`, compares the complete canonical plan, repeats fresh DMI/BIOS/USB discovery, and binds acquisition to the selected bus/port/serial identity. It opens that one `187c:0551` device and, before any handoff, revalidates its identity, HID interface `00` (`03/00/00`), interrupt endpoints `0x01`/`0x81`, and 33-byte packet size. An unknown driver state fails closed. If interface 0 is inactive, it is claimed directly; if it is active, only that interface on the already selected device is detached once, claimed, revalidated with an inactive driver, then released and reattached when finished. It never detaches `usbhid` globally or touches another interface or USB device. Every successful, failed, or short write path explicitly releases the claim and reattaches only when this backend detached it; `Drop` is only a best-effort fallback. Release or reattach failure is reported and preserves completed transfer progress, so it is never reported as success. It writes each of the 34 packets exactly once with the existing 500 ms per-write timeout and aborts on the first backend error or short transfer without retrying or sending later packets. The ordinary timeout exposure is up to about 17 seconds plus scheduling and acquisition. A later failure can leave a partial profile; persistence is unknown and there is no readback or restore.
...
Doctor reports keyboard and chassis technical readiness separately. Keyboard readiness requires exact supported DMI/BIOS, the pinned descriptor, and an interface-00 hidraw node with effective read/write access. Chassis readiness remains pre-open evidence only: exact supported DMI/BIOS, exactly one `187c:0551`, a USB bus number, a nonempty physical port path, and interface `00` when interface identity is available. `ready_for_writes` is true only when both controller profiles are technically ready. AW-ELC opened identity, HID endpoints, driver state, and access are revalidated only during acquisition; doctor never opens, detaches, claims, releases, or reattaches USB interfaces, and no readiness boolean is physical target validation.
...
[`contrib/udev/70-alienrgb.rules`](contrib/udev/70-alienrgb.rules) narrowly assigns the static `alienrgb` group, retains `TAG+="uaccess"`, and uses mode `0660` for keyboard hidraw `0d62:d2b1` interface `00` and AW-ELC USB `187c:0551` with its expected HID interface signature. It does not match unrelated hidraw or USB devices. Descriptor hashes, physical topology, endpoints, and driver state remain runtime-only checks. Installation and rule reload are manual; see [`contrib/udev/README.md`](contrib/udev/README.md). The project does not install or reload rules.

This checkout's rule adds `GROUP:="alienrgb"`; any previously installed copy is no longer byte-identical and does not provide pre-login group access. Reinstall this checkout's rule and reload/trigger udev as documented in [`contrib/udev/README.md`](contrib/udev/README.md) before enabling pre-login restoration. On the current confirmed host, `/dev/hidraw1` is root-owned mode `0660`, while its active POSIX ACL grants the current user effective read/write access. The guarded non-root transport has physically validated the exact static-red one-key, one-frame, two-frame, and full-catalog operations recorded above. Only Escape, F1, W, and Space establish individual keyboard address identity; the uniform `all` result establishes full-set coverage, not every individual mapping. Do not run `alienrgb` with `sudo`.
