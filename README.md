...
The optional systemd integration in [`contrib/systemd`](contrib/systemd/README.md) runs as the named non-root instance user. It becomes active for `sleep.target` and restores in `ExecStop` after suspend or hibernate. The helper validates the state strictly, performs a bounded sequence of read-only `doctor --json` readiness checks, and then executes the complete confirmed `set-all` command exactly once with every existing confirmation flag. It does not retry transport. Because there is no reliable RGB readback, restoration always reapplies the entire recorded profile. Before the first fully successful `set-all`, no state exists and post-resume handling exits successfully without writing hardware.
